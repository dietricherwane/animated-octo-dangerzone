class Devise::RegistrationsController < DeviseController
  ...
  def create
    ...
    if not_a_number?(@phone_number) then @error_messages << "Le numéro de téléphone est un nombre sans espaces"; @phone_numbercss = "row-form error" else @phone_numbercss = "row-form" end
    ...
    if @password.blank? or @password.length < 8 then @error_messages << "Veuillez entrer un mot de passe d'au moins 8 caractères"; @passwordcss = "row-form error" else @passwordcss.blank? ? @passwordcss = "row-form" : false end
    if @profile_id.blank? then @error_messages << "Veuillez choisir un profil"; @profile_idcss = "row-form error" else @profile_idcss.blank? ? @profile_idcss = "row-form" : false end   
  ...
  def edit
    ...
    @profiles = Profile.where("published IS NOT FALSE")
  	@firstnamecss = @lastnamecss = @phone_numbercss = @mobile_numbercss = @profile_idcss = "row-form"
    render :edit
  end
  ...
  def update_profile
    ...
    if @error_messages.blank?
      @user.update_attributes(firstname: @firstname, lastname: @lastname, phone_number: @phone_number, mobile_number: @mobile_number, profile_id: @profile_id)
